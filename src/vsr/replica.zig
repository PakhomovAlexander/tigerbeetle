...
pub fn ReplicaType(
    ...
    return struct {
        ...
        fn on_start_view(self: *Self, message: *const Message.StartView) void {
            ...
            if (message.header.commit > self.op_checkpoint_next_trigger() and (
            //     Cluster is far ahead, replica's WAL no longer intersects primary's WAL
                message.header.op > self.op_prepare_max() or
                // Cluster is a bit ahead. The replica should be able to repair WAL, unless the cluster
                // got to the checkpoint via state-sync.
